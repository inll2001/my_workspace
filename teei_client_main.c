...
long teei_service_init_first(void)
{
	...
	printk("[%s][%d] begin to create cancel command buffer!\n", __func__, __LINE__);
	cancel_message_buff = create_cancel_fdrv(CANCEL_MESSAGE_SIZE);
	if (cancel_message_buff == NULL) {
		printk("[%s][%d] create cancel buffer failed!\n", __func__, __LINE__);
		return -1;
	}
	if (soter_error_flag == 1)
		return -1;
...
long teei_service_init_second(void)
{
...
#ifdef TUI_SUPPORT
	printk("[%s][%d] begin to tui display command buffer!\n", __func__, __LINE__);
	tui_display_message_buff = create_tui_buff(TUI_DISPLAY_BUFFER, TUI_DISPLAY_SYS_NO);
        if (tui_display_message_buff == NULL) {
		printk("[%s][%d] create tui display buffer failed!\n", __func__, __LINE__);
		return -1;
	}
	...
	printk("[%s][%d] begin to tui notice command buffer!\n", __func__, __LINE__);
	tui_notice_message_buff = create_tui_buff(TUI_NOTICE_BUFFER, TUI_NOTICE_SYS_NO);
        if (tui_notice_message_buff == NULL) {
		printk("[%s][%d] create tui notice buffer failed!\n", __func__, __LINE__);
		return -1;
	}
	if (soter_error_flag == 1)
		return -1;
